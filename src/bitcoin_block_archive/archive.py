...
def archive_block(
    ...
    if before != _source_signature(block_file, "while calculating checksum"):
    ...
    first = BlockReference(first_hash, block_height(config, first_hash))
    last = BlockReference(last_hash, block_height(config, last_hash))
    ...
    if before != _source_signature(block_file, "during upload"):
        raise ArchiveError(f"{block_file} changed during upload")
