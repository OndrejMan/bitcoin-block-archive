...
def test_manifest_is_contiguous_and_carries_tip_height(config: Config) -> None:
    ...
    for number, height in ((0, 100), (1, 250)):
        write_marker(
            ...
            f"{number + 1:064x}",
            42,
            first_block=BlockReference(f"{number + 2:064x}", height - 1),
            last_block=BlockReference(f"{number + 3:064x}", height),
            source_signature=FileSignature(42, 0, 0),
        )

    manifest = build_manifest(config, archived_max_height=250)
...
def test_publish_manifest_writes_local_snapshot_and_uploads_it(config: Config) -> None:
    ...
    write_marker(
        ...
        "a" * 64,
        1,
        source_signature=FileSignature(1, 0, 0),
    )
    client = FakeClient()
...
def test_explicit_coverage_overrides_a_file_local_height(config: Config) -> None:
    config.state_dir.mkdir()
    write_marker(
        config,
        config.block_dir / "blk00000.dat",
        "a" * 64,
        1,
        last_block=BlockReference("b" * 64, 200),
        source_signature=FileSignature(1, 0, 0),
    )
